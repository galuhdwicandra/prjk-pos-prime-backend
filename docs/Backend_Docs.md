...
_Dihasilkan otomatis: 2025-10-29 16:13:24_  
**Root:** `/home/galuhdwicandra/projects/prjk-prime/backend`
    ...
    public function note(int $id)
    {
        $delivery = Delivery::with([
            'order'   => fn($qo) => $qo->select('id', DB::raw('kode as code'), 'cabang_id', 'subtotal', 'discount', 'grand_total', 'paid_total', 'status', 'created_at')
                ->with([
                    'items:id,order_id,name,qty,price,note',
...
class CheckoutService
{
    ...
    private function recordPayment(Order $order, array $p): void
    {
        ...
        // Default status
        $defaultStatus = in_array($method, ['CASH', 'QRIS'], true) ? 'SUCCESS' : 'PENDING';
        ...
        $pay->status       = $status;
        $pay->ref_no       = $p['ref_no'] ?? null;
        $pay->payload_json = $p['payload_json'] ?? null;
        $pay->paid_at      = isset($p['paid_at']) ? Carbon::parse($p['paid_at']) : now();
        $pay->save();
    ...
    private function postAccountingForPayment(Order $order, Payment $pay): void
    {
        ...
        $isBank = strtoupper($pay->method) === 'TRANSFER';
    ...
    public function buildSuratJalanHtml(Delivery $d): string
    {
        ...
        $d->load([
            'order' => function ($qo) {
                $qo->select(
                    'id',
                    'kode',          // pakai kolom asli, bukan alias
                    'cabang_id',
                    'subtotal',
                    'discount',      // ✅ kolom yang benar
                    'grand_total',
                    'paid_total',
                    'status',
                    'created_at'
...
HTML;
    ...
    {
        $d->loadMissing(['branch']);
        $branchCode = $d->branch->code ?? 'BR';
        return 'SJ-' . $branchCode . '-' . now()->format('Ymd') . '-' . str_pad((string)$d->id, 5, '0', STR_PAD_LEFT);
    }
